...
6. **Trade mogelijk** (Range ≥ `minRangeRR` × R **én** R ≤ `maxRTicks` ticks): een blauwe stippellijn met de TP-waarde wordt getekend, met label "TP (1.2 R/R): ... · Range x.xxR" — zelfde stijl als de SL-lijn. De Range wordt daarbij ook uitgedrukt in R (Range ÷ R, naar beneden afgerond op 2 decimalen), zodat direct zichtbaar is hoe ruim de trade de check haalt.
