...
| Minimaal benodigde R/R-ruimte in de Range (`minRangeRR`) | 1.5 | De beschikbare ruimte tussen Resistance/Support en de bijbehorende Target-lijn moet minimaal `minRangeRR × R` zijn, anders is de trade "niet mogelijk" (geen TP-lijn, wel een label met de reden). | Is onafhankelijk van de `maxRTicks`-check hieronder; beide voorwaarden kunnen los van elkaar (of tegelijk) een trade blokkeren. |
